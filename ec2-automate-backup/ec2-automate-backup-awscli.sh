...
create_EBS_Snapshot_Tags() {
  ...
  fi #if $name_tag_create is true then append ec2ab_${ebs_selected}_$current_date to the variable $snapshot_tags
...
for ebs_selected in $ebs_backup_list; do
  ...
  ec2_snapshot_resource_id=$(aws --profile $profile ec2 create-snapshot --region $region --description $ec2_snapshot_description --volume-id $ebs_selected --output text --query SnapshotId 2>&1)
  if [[ $? != 0 ]]; then
